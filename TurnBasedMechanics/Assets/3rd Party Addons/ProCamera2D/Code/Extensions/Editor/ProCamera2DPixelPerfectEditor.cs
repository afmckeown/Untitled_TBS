...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2DPixelPerfectEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Pixels per Pawn
            _tooltip = new GUIContent("Pixels per Unit", "How many pixels in a sprite correspond to one Pawn in the world");
            ...
            EditorGUILayout.BeginHorizontal();
            _tooltip = new GUIContent("Snap Movement to PointExtensions", "If checked, the the sprites will snap to the grid. Might create some stuttering on your camera targets, especially if you're using a large grid and a follow smoothness greater than zero.");
            ...
            _tooltip = new GUIContent("Draw PointExtensions", "If checked, the camera will draw a pixel grid. 'Gizmos' button must be enabled on the Game window.");
            EditorGUILayout.PropertyField(serializedObject.FindProperty("DrawGrid"), _tooltip);

            if (proCamera2DPixelPerfect.DrawGrid)
            {
                _tooltip = new GUIContent("PointExtensions Color", "");
                EditorGUILayout.PropertyField(serializedObject.FindProperty("GridColor"), _tooltip);
            }

            EditorGUILayout.EndHorizontal();

            // PointExtensions density warning
            if(proCamera2DPixelPerfect.DrawGrid && proCamera2DPixelPerfect.GridDensity < 4)
                EditorGUILayout.HelpBox("PointExtensions density is too high to draw, so we're skipping it to avoid performance issues with the editor.", MessageType.None, true);
